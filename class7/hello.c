/* hello.c linked list example
 * compile with gcc hello.c -o program
 * from Phill @ SCN - converted to standard C
 */
...
struct LinkNode* MakeNode(int info) {
...
int main() {
    ...
    struct LinkNode* cursor = NULL;
    struct LinkNode* previous;
    struct LinkNode array[5];
    struct LinkNode* NewNode;
    int x;

    /* Initialize the array of LinkNodes */
    for (x = 0; x < 5; x++) {
        array[x].data = 0;
        array[x].next = NULL;
    }

    ptr1 = arr; /* ptr1 points to the first element (arr[0]) */
    ...
    ptr2 = &arr[3]; /* ptr2 points to the fourth element (arr[3]) */
    ...
    ptr3 = &arr[0]; /* ptr3 points to the first element of arr */
    ...
    for (x = 0; x < sizeof(array) / sizeof(struct LinkNode); x++) {
        printf("array data: %d\n", array[x].data);
